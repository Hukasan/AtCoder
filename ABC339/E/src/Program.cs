...
namespace AtCorder
{
    public class Program : ProgramBase
    {
        public static void Main()
        {
            ...
            void CalcSum((int X, int Y) start, (int X, int Y) target)
            {
                var dif_x = target.X - start.X;
                var dif_y = target.Y - start.Y;

                // 斜め移動できる場合はしましょう。
                if (dif_x != 0 && dif_y != 0)
                {
                    var naname = Math.Min(Math.Abs(dif_x), Math.Abs(dif_y));
                    if (dif_x < 0)
                        dif_x += naname;
                    else
                        dif_x -= naname;
    ...
    public class ProgramBase
    {
        ...
        public static IEnumerable<List<string>> GetLines(int? MaxCnt = null)
        {
            var cnt = 0;
            while (true)
            {
                if (MaxCnt != null && cnt >= MaxCnt) yield break;
                var line = Line();
                cnt++;
                if (line == null || line.Count == 0) yield break;
                yield return line;
            }
        }
        /// <summary>
